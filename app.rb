...
get_or_post '/' do
  ...
  data = RestClient.get "#{DB}/_design/viewall/_view/viewall?limit=25&descending=true"
  puts "yeah"
  @result = JSON.parse(data)['rows']
 erb :index
end

#when user moves a note, it generates a locupdate event shich is then pushed to all conected users
post '/locupdate' do
  puts "push push"
